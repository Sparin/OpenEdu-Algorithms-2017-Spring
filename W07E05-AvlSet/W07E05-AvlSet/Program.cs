    ...
    class Program
    {
        static void Main()
        {
            ...
            for (int i = 1; i < stdin.Length; i++)
            {
                ...
                {
                    case "A":
                        AvlTree<long> s = AvlTree<long>.Search(root, long.Parse(temp[1]));
                        if (s == null)
                            root = AvlTree<long>.Insert(root, new AvlTree<long> { Key = long.Parse(temp[1]) });
                        Console.WriteLine(AvlTree<long>.GetBalance(root));
                        break;
                    case "D":
                        AvlTree<long> t = AvlTree<long>.Search(root, long.Parse(temp[1]));
                        if (t != null)
                            root = AvlTree<long>.Remove(t);
                        Console.WriteLine(AvlTree<long>.GetBalance(root));
                        break;
                    case "C":
                        AvlTree<long> x = AvlTree<long>.Search(root, long.Parse(temp[1]));
                        if (x != null)
                            Console.WriteLine("Y");
                        else
                            Console.WriteLine("N");
